        ...
        while(True):
            ...
            if(len(temp_vacancy_list) != 0):
                ...
                if(current_row < self.board_size - 1):
                    ...
                    solutions.append(self.extract_queen_positions())

                    # print(f"Solution {solution_count} : {self.extract_queen_positions()}")
...
def main():
    ...
    def throw_solutions(thread_list):
    ...
    board_list = [Board(size) for _ in range(size)]
    
    x = 1
    while(x < len(board_list)):

        j = 0
        while(j < x):
            board_list[x].board[0][j] = True
            j += 1
    ...
    for board in board_list:
        threads_list.append(threading.Thread(target = board.find_positions_n_queens))
