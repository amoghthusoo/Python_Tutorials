...
class Board:
    ...
    def find_positions_n_queens(self) -> list[list[int]]:
        ...
        while(current_row < self.board_size):

            temp_vacancy_list = self.find_vacancies(current_row)    # passing of current_row can be ignored
        ...
        for _ in range(3):
            ...
            queen_positions = self.extract_queen_positions()
            if(queen_positions not in solution):
                solution.append(queen_positions)
        
        self.rotate_board_clockwise()
        self.flip_board_vertical()
    ...
    def rotate_board_clockwise(self):
        ...
        temp = [[None for i in range(self.board_size)] for i in range(self.board_size)]
    ...
    def flip_board_vertical(self):

        i = 0
        while(i < self.board_size):

            j = 0
            while(j <= self.board_size//2 - 1):
                
                self.board[i][j], self.board[i][self.board_size - 1 - j] = self.board[i][self.board_size - 1 - j], self.board[i][j] 
